...
{
if(playerHasItem(charm, 5) && playerHasItem(item, 5) && playerHasItem(shards, amountofshard * 5) && playerHasItem(pouch, 5) && c.playerLevel[22] >= req) {
deleteItem(charm, 5);
deleteItem(item, 5);
deleteItem(shards, amountofshard * 5);
deleteItem(pouch, 5);
return true;
} else {
c.sendMessage("You need the following items: 5x "+c.getItems().getItemName(charm)+" to make 5 pouches.");
...
return false;
	 ...
	 public void makeDreadfowl(int itemUsed, int useWith) {
...
public void makeSpiritWolf(int itemUsed, int useWith) {
	switch(itemUsed) {
	case 2859:
req = 1;
useWith = pouch;
