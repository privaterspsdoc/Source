...
public class AttackNPC {
	...
	public static void applyNpcMeleeDamage(Client c, int i, int damageMask,
		...
		if (NPCHandler.npcs[i].npcType == 6222 
			&& !c.usingCross
			&& !c.usingBow 
			&& !c.usingMagic
			&& !c.getCombat().usingCrystalBow()
			&& !c.usingOtherRangeWeapons
			&& !c.usingToxicBlowpipe) {
				c.getCombat().resetPlayerAttack();
				c.sendMessage("Kree'arra can only be damaged with ranged or magic weapons.");
			return;
		}
	...
	public static void attackNpc(Client c, int i) {
		if (NPCHandler.npcs[i] != null) {
			...
			if (c.attackTimer <= 0) {
				...
				c.usingCross = c.playerEquipment[c.playerWeapon] == 11785
				...
				if ((!c.goodDistance(c.getX(), c.getY(),
						...
						|| (!c.goodDistance(c.getX(), c.getY(),
								...
								&& !c.usingBow && !c.usingMagic && !c.usingToxicBlowpipe))
						|| (!c.goodDistance(c.getX(), c.getY(),
								NPCHandler.npcs[i].getX(),
								NPCHandler.npcs[i].getY(), 1) && (!c.usingOtherRangeWeapons
								&& !c.getCombat().usingHally() && !c.usingBow && !c.usingMagic && !c.usingToxicBlowpipe))
						|| ((!c.goodDistance(c.getX(), c.getY(),
								NPCHandler.npcs[i].getX(),
								NPCHandler.npcs[i].getY(), 8) && (c.usingBow || c.usingMagic ||c.usingToxicBlowpipe)))) {
					c.attackTimer = 2;
					return;
				}
				...
				c.faceUpdate(i);
				//System.out.println("Npc " + NPCHandler.npcs[i].npcType + " player " + c.playerId);
				NPCHandler.npcs[i].underAttackBy = c.playerId;
				...
				if (c.usingOtherRangeWeapons && !c.usingMagic && !c.usingBow && !c.usingToxicBlowpipe) {
					c.followId2 = NPCHandler.npcs[i].npcId;
					c.getPA().followNpc();
					c.rangeItemUsed = c.playerEquipment[c.playerWeapon];
				...
				if (c.usingToxicBlowpipe && !c.usingMagic && !c.usingBow && !c.usingOtherRangeWeapons) {
					...
					c.getCombat().fireProjectileNpc();
						...
						c.sendMessage("Your blowpipe is out of ammunition!");
					}
				}
