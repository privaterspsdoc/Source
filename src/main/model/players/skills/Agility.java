	...
	public void agilityCourse(final Client c, final int objectType) {
		...
		case 2295:
			...
			CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
				...
				public void stop() {
					resetAgilityWalk(c);
					c.getPA().addSkillXP((int) 10.5 * Config.AGILITY_EXPERIENCE,
							c.playerAgility);
					logBalance = true;
					doingAgility = false;
				}
		...
		case 2312:
			...
			CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
				...
				public void stop() {
					resetAgilityWalk(c);
					c.getPA().addSkillXP(10 * Config.AGILITY_EXPERIENCE,
							c.playerAgility);
					balanceRope = true;
					doingAgility = false;
				}
		...
		case 2315:
			...
			c.getPA()
					.addSkillXP(9 * Config.AGILITY_EXPERIENCE, c.playerAgility);
			treeBranchDown = true;
			break;
		case 2286:
			doingAgility = true;
			...
			CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
				...
				public void stop() {
					...
					c.getPA().addSkillXP(11 * Config.AGILITY_EXPERIENCE,
							c.playerAgility);
			...
			CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
				...
				public void stop() {
					...
					if (logBalance && obstacleNetUp && treeBranchUp
						...
						if (c.runsDone < 400) {
							c.runsDone += 1;
							c.sendMessage("You have now completed " + c.runsDone + " of the tree gnome agility course.");
						}
						c.getPA().addSkillXP(47 * Config.AGILITY_EXPERIENCE,
								c.playerAgility);
								c.getPA().resetAgility();
						c.sendMessage("You have completed the full gnome agility course.");
					} else {
						c.getPA().addSkillXP(7 * Config.AGILITY_EXPERIENCE, c.playerAgility);
						c.getPA().resetAgility();
					}
					logBalance = false;
					obstacleNetUp = false;
